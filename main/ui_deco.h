 ...
 *           b = 虚线节距（0 = 实线）。虚线是制图里的"推断/参考"语义，
 *           LVGL 原生支持，不额外要钱。
 *   ARC     (x,y) = 圆心，w = 半径，h = 线宽，a/b = 起止角
 *           （度，0° 在 3 点钟方向，顺时针）
 *   TRI     (x,y) = 顶点1，(w,h) = 顶点2 相对，(a,b) = 顶点3 相对；实心
 *   DOT     (x,y) = 圆心，w = 半径，h = 线宽（0 = 实心圆）
 *   CHEV    (x,y) = 尖端，(w,h) = 一臂位移（另一臂 x 向镜像），a = 线宽
 *   CROSS   (x,y) = 中心，w = 臂长，h = 线宽，a: 0 = 正十字，1 = 斜叉
 *   DIAMOND (x,y) = 中心，w = 半宽，h = 半高（0 则取 w）；实心
 *
...
typedef enum {
    ...
    DSHAPE_TRI,
    DSHAPE_DOT,
    DSHAPE_CHEV,    /* 人字 / 箭头：(w,h) 是一臂，另一臂 x 向镜像 */
    DSHAPE_CROSS,   /* 十字或斜叉：w = 臂长，h = 线宽，a: 0=+ 1=× */
    DSHAPE_DIAMOND, /* 实心菱形：w = 半宽，h = 半高（0 则取 w） */
} deco_kind_t;
