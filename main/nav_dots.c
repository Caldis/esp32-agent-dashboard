...
/* 三个键在屏幕水平方向的 25% / 50% / 75%，即距中心 -116 / 0 / +116。
 * 点位直接对应物理按键的位置，所以间距不是审美选择，是映射关系。 */
#define DOT_GAP      (SCREEN_W / 4)
/* 尽可能上移：让出中间的连接状态指示器（conn_dot，y 24..40）。点在
 * 8..18，与它有 6px 净距，且在 y=8 处即使按圆角半径 100 算，可用横向
 * 范围也有 x 61..405，25%/75% 的 116/349 安全落在里面。 */
#define DOT_Y          8
