...
/* 三个键在屏幕水平方向的 23% / 50% / 77%，即距中心 -126 / 0 / +126
 * （27/100 屏宽）。点位直接对应物理按键的位置，所以间距不是审美选择，
 * 是映射关系——拉宽后三点的分布更接近手指在机身上的实际间距。
 * 边界检查：左点圆心 x=107，墨迹约 102..112。面板是圆角矩形，圆角半径 RING_RADIUS(60)，
 * x>60 的位置整条高度都可用，所以 y=8 处安全。 */
#define DOT_GAP      ((SCREEN_W * 27) / 100)
/* v6.9: 从 y=8 下移到 20。
 * 原来贴到最上沿，是为了让开顶部中央那颗连接状态圆点（y 24..40）。那颗点
 * 已经被边缘辉光取代并删除了，这条约束随之消失；取而代之的新约束是【辉光
 * 自己】——它的环带从 UI_VIS_INSET 起向内 RING_N*RING_STEP+RING_WIDTH，
 * 掉线时常驻，压在 y=8 的点上就是用户看到的重叠。
 * 20 让点落在辉光带（3..17）之下，仍远在顶钟（y=56 起）之上。 */
#define DOT_Y         20
