...
/* v7.3: `a` 的最新实测（`?vis` 标定尺，见 ui_calib.c）。第一轮候选
 * 0/3/6/9 里【没有一个】画出完整一圈——用户看到的是四个框各缺一半，
 * 这恰好排除了这四个值本身，把 a 夹到 {1,2,4,5}。取 3 作估计值，最坏
 * 偏差 ±2px。
 *
 * 这个数只有一双眼睛能给：`?dump` 渲染的是整个 480×480 帧缓冲缩放到
 * 466（实测：k=0 的框右缘落在截图第 451 列 = 465×466/480），不是可见区
 * 裁剪——物理上哪一列先点亮根本不在帧缓冲里。
 *
 * 贴边元素不要去"精确贴住"这条量不准的边：让最外层【溢出屏外】，由
 * 物理边缘去裁它（见 ui_glow.c 的 GLOW_BLEED）。这样 a 在 0..7 之间取
 * 任何值都不会露出缺口——把量不准的常数变成设计上的非问题。 */
#define UI_VIS_ORG       3
