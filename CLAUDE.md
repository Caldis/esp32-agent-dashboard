...
## 机能装饰层 ui_deco (v7.5 静态 → v7.6 编排引擎)
`main/ui_deco.c` — 一层纯装饰的几何标记（四角取景框、结构线、刻度组、
分段条），铺在场景内容【之下】，不承载功能、不吃触摸、不进演员表。
目前只挂在 clock；扩展到别的场景 = 再写一张谱，不动引擎。

- **比例是这套东西的核心约束，不是审美问题**。按 ui_type.h 的视距口径
  推：最小笔画 4 px（低于 1 arcmin 就消失），块状元素特征尺寸 ≥24 px
  （读作"形状"需约 8 arcmin）。参照系 Marathon 在 27"/0.5 m 上的 8 px
  细节 = 12.8 arcmin，在这块屏 @0.8 m 要 **33 px** 才等效 —— **缩放
  4x、密度只有它的 1/20**（480 宽只放得下 15 个元素，它能放 320 个）。
  照抄密度必然糊成灰雾。例外：**线可以细，块不能小**（长条形的检测
  阈值远低于点状，2 px hairline 在 1 m 处仍可见）。
- 机能感来自【分布方差】不是数量：沿结构线聚集 + 大片留白。v7.5 实机
  栽过一次 —— 底部 12 格等宽等距横跨 354 px，读作 progress bar 而非
  刻度尺，砍到 6 格 + 偏置 + 右端另起一组性质相反的静态块才对。
- **运动语法是机械的**，与内容层的 spring 构成复调（"人在用一台机器"）。
  原型分野只有一条：结构元素(BRACKET/LINE)连续【生长】，数据元素
  (BLOCKS/TICKS/GAUGE)离散【点亮】。进度量 p∈[0,1000] 统一表达完成度，
  **出场不需要单独实现** —— p 退回 0 时生长类自末端收回、点亮类自最后
  一格熄灭，后进先出自动成立。
- **持续动效必须跑在 idle 刷新档（66 ms）上**，否则等于把设备永久钉在
  高刷。所以停留期全部是 200 ms 一拍的离散事件，且相位在【暂停期钳住】
  （ph 不变 = 签名不变 = 不失效），只有入场/出场才 hold 高刷。实测静置
  busy_pct 5.1%→5.7%，period_ms 保持 66（没有泄漏高刷档）。
- 失效【按元素】而非整层：每个元素一个 bbox，只有 (p, ph) 变了的才失效。
- 颜色：**无彩色**。设备级契约是"颜色跟随 STATE 不跟随页面"，装饰引入
  第三种彩色就是拿状态契约换视觉。只用主题 text 色 + 明度分层。
- **装饰用【时间维度】表达状态**（v7.8）。不能碰颜色，但节奏是空的：
  `ui_deco_set_pace()` 接 `agent_state_attention()`（2=有人等你 / 1=在跑 /
  0=空闲），改的只是 opa 乘数（180/255/320）与两个周期常数（重锁定
  45/30/12 拍、扫描静默 20/12/4 拍）。零新增形状、零新增 draw call，
  而两档的实机观感差异非常明显。颜色归内容层、节奏归装饰层，两层各说
  各的同一件事。
  · 代价记在这：alert 档静置 render 9.9→15.4 ms（**+55%**），busy_pct
    10.6%→16.2%；但 drawn/overrun/period_ms 全部不变（0 掉帧、仍在 66 ms
    档）。alert 通常只持续到用户抬头看一眼，值这个钱。
  · 测这个要小心：推快照【那一瞬】的 inval 会冲到 23 万 px/帧（姿态切换
    + ambient 簇滑动 + chip 出现），那是一次性事件不是稳态——等 3 s 再
    `?perf` 清窗口，否则会把内容层的账算到装饰头上。
- **`int_free` 随运行时长下降与装饰层无关**（v7.8 定责，此前一直悬着）：
  同一台设备跑 16 次转场，`?deco 0` 时 int_free 33903→32047（约 116 B
  /转场），`?deco 1` 时 32047→32047 一点没降。是转场路径的既有消耗。
  重启即恢复，所以不是硬泄漏；但跌到 ~31 KB 时 `?perf` 会回不全字段
  （trans_bench 报 `KeyError: frame_ms`），长会话里留意一下。
- 调试：`?deco 0|1` A/B、`?deco g <0-255>` 全局 opa 增益（实机扫存在感）、
  `?deco f <ms>` 定格入场包络（入场 0.7 s 而一张 480 截图要 5 s，动态
  过程只能这样验证，同 `?glow`）、`?deco in|out` 手动播一次。
- 转场接入靠 `trans_profile_t.on_intro / on_outro`（v7.6 新增）。自绘图层
  没有可动的对象能当演员——一个对象里装着几十个形状，位移它等于位移
  全屏——所以框架只给"该演了"这个时机，怎么演由图层自己决定。
- **入场必须给演员让路**（v7.6.1，最贵的一课）。三张谱上齐后一次同固件
  背靠背 A/B 测出 render **+10%~+58%**、四行破门，而 `drawn` 反而下降
  ——每帧变贵不是帧数变多。根因不是装饰那 3.7k px 墨迹，是**它被演员的
  大失效区连累**：演员从屏外飞入时失效区可达 400x200，装饰在最底层，
  落在里面的十几个矩形每帧都得重画，每个都是一次 draw-task 创建+调度。
  静置时 AABB 预判挡得住，转场时挡不住。把入场整体推后到演员落座之后
  （`ui_deco_intro` 的 lead = IN_MS），两个昂贵窗口不再重叠 → +23% 封顶。
  **推论：本文件别处那条"稀疏几何 fill 比 blit 便宜"只在失效区小时成立。**
- 拍频要和刷新档【同拍】：入场 66 ms = ui_motion 静止档周期，于是本层
  完全不持有高刷档。之前 hold 高刷（16 ms）却只有 50 ms 一拍，2/3 的
  刷新周期找不到脏区域纯空转——这是 v7.0 那条教训的镜像（当年是动画慢
  于刷新而丢帧）。改完 +23% → **+7%，六行全进门**，还省电。
- 失效粒度按原型分：生长类签名用连续的 p，点亮类用 `p*rn/1000`（只在
  跨格时变）。用 p 当签名会让 6 格元素在 270 ms 入场里失效 8 次而非 6 次。
- **屏幕最外圈约 46 px 属于 ui_glow**（5 层 x step8+width7 + 运动外溢），
  装饰不能进。weather 谱第一版把基线摆在 y=447，实机上整条消失；改到
  插画底(278)与五日条带顶(306)之间那条缝，顺带成了两区的分隔线。
- A/B 要用【同固件背靠背】(`?deco 0` 跑一轮 → `?deco 1` 再跑一轮
  `--compare`)，不要和几天前的 baseline 比：那里面混着别的优化和不同的
  实时天气，曾让我把 +58% 的真回归读成 +0%。两轮之间要留 10 s 以上，
  否则 `?perf` 会来不及回全字段，trans_bench 报 `KeyError: frame_ms`。

### 三张谱的差异化与姿态适配 (v7.7)
- **角标形态是三张谱的身份**，不能共用：它们原本是同一份 8 矩形拷贝，
  三个页面因此像同一张模板。现在按语义分化——clock 是 `L + 分度延伸`
  （精密读数，12 矩形）、dashboard 是 `方括号 [ ]`（数据通道，12 矩形）、
  weather 是 `单臂交替`（最轻、不闭合成框，4 矩形）。矩形数本身就是
  密度声明。
- **姿态掩码** `deco_elem_t.mask`：dashboard 的 ambient/fleet、clock 的
  大钟/推送卡。进退场复用同一套 p 语法，所以换姿态是长出来/收回去。
  · fleet 姿态下中央夹持括号 + 两条侧轴退场——几何上放得下（卡片占
    x28..452，两侧还有 28 px），但四行卡片已填满画面，这是**负空间**
    的决定，不是性能的。
  · clock 推送卡弹出时顶部带（60..71）必须让位：大钟退到顶槽位后 48px
    墨迹落在 61..109，完全重叠。第一版没做区分，实机截图里"00:29"正压
    在分段块中间，当时误读成巧合的好看。
- **`DECO_DEFER`：姿态专属元素不参与转场入场**，等落定后补上。转场演的
  是通用骨架；补充元素跟着涌入既加重最贵的窗口（实测 +23%~+43%、四行
  破门），叙事上也含糊。落定后屏幕静止，它们的失效区只剩自己那一小块，
  AABB 预判重新生效 → **-6%~+12%，六行全进门**。

### 姿态适配、动态几何与验收门 (v7.8)
- **dashboard 有三个姿态不是两个**：PLAIN（无 chip，簇 y160、环墨顶 172）
  / CHIP（awaiting，整簇上移到 y133、环墨顶 145、chip 墨底 ~345）/ FLEET。
  ambient 的两个子姿态布局差 27 px，同一个让位量适配不了两者。
- `deco_elem_t.dy_a / dy_b` = 姿态 A/B 下整组的 y 偏移；`span_slot` = 按
  槽值（千分比）缩放元素进度，就是本层的【动态几何】。后者直接乘进 p，
  于是生长类（弧扫多少角）与点亮类（点到第几格）同时受益。首个用例：
  clock 右下角那条同心弧随分钟生长，一小时扫完——四角看似对称只有一角在动。
- **bbox 存基准位置，失效时按当前姿态平移**。曾按"两姿态并集"存，两条带
  的盒子凭空高 18-26 px，常态每次失效都在为用不到的位置买单（实测
  dashboard->clock render **+30%**）。姿态切换时失效新旧两处即可，那是
  稀疏事件。
- `ui_deco_alert()`：一道强调按入场顺序快速扫过全层。给 attention 升到
  ALERT 那一下用——agent 交还回合是这台面板的唯一职责，装饰不该只是事后
  换个节奏。三个场景都在 tick 里检测上升沿。

### `tools/perf/deco_audit.py` —— 负空间验收门
`& ./tools/with_port.ps1 { python tools/perf/deco_audit.py }`
逐姿态（5 个）截图并量出装饰墨带与内容墨带之间的每一段间距，超限 exit 1。
- **怎么把同色的两层分开**：`?deco 0/1` 各截一张，**相减**即得装饰层的
  纯图像。`?deco` 当初是为 A/B 加的，这里让视觉本身变得可测量。
- 三处校准是踩出来的，改判据前先读懂它们：
  · `CONTENT_TH` 必须低到吃下 fleet 卡片底色（0x1C1814，灰度 ~26），否则
    卡片被滤掉，"上方最近的内容"越过它多算 250 px；
  · `CONTENT_MIN_INK` 必须低到留住呼吸环——它是【描边】，每行只切到两段
    各 ~3 px，而它正是上带下方要让位的对象；
  · 墨带**必须带 x 跨度**：行投影丢掉水平信息，而 dashboard 的上带是与
    顶部时间【同行分列】的，没有 x 重叠检查会把"并排"误报成"距内容 3 px"。
- 外圈 `EDGE_SKIP=46` 掐掉，那是 ui_glow 的地盘，它一直在呼吸。
- 豁免写在 CASES 表里并注明理由（clock 顶带刻意贴顶缘），不要放宽全局阈值。
- 它抓到过我目测漏掉的缺陷：以为时间墨底在 90，实际 107，上带距它只有
  1 px。**目测量不准，让机器量。**
- **任何改设备状态的脚本都必须 `finally` 还原**。本脚本每个姿态要
  `?deco 0` -> 截图 -> `?deco 1` -> 截图，一旦在中间被打断（超时、
  Ctrl+C、主机死机），设备就停在"装饰关闭"——而设备是 USB 供电的，
  主机重启它也不断电，那个状态一直留着，看起来像"装饰丢了"。踩过一次。
- 装饰元素**没有语义就该删**，哪怕它占的是空地：dashboard 曾有左右两条
  侧轴，不夹持任何东西（呼吸环已由中央那对括号夹住），形态又和 clock 的
  侧轴几乎一样——正是三张谱差异化要消除的重复。删掉后 elems 21->17。

### 形状系统 (v7.7)
v7.7 之前整层只有轴对齐矩形，画面里因此只有横竖两个方向。现在
`deco_shape_t.kind` 支持 RECT / LINE（任意角度，`b`>0 走 LVGL 原生虚线）
/ ARC / TRI / DOT / CHEV（人字）/ CROSS（十字或斜叉）/ DIAMOND，参数
复用同一组字段（见 ui_deco.h 的对照表）。
- **能用 rect 表达的形状不要交给 draw_line**：AA 线是这块板上最贵的绘制
  之一（weather 那 30 条线的教训）。`CROSS` 的正十字分支因此走两个轴
  对齐矩形，只有斜叉才真画线；作为对位标记 + 与 × 语义等价，所以谱里
  默认用正十字。
- 虚线的语义是"推断/参考"，weather 的中缝右半段（五日预报侧）用它，
  与左半实线（今日实测）刚好对上——这是本层少见的形状与内容同构处。
- 点缀类形状（人字、十字、菱形、圆环）一律标 `DECO_DEFER`：它们是补充
  不是骨架。
- **看 overrun 不要只看 render_avg**：形状扩到 8 种后 render_avg +11%
  （一行 +16% 踩门），但同次 A/B 的 overrun 总数 140→134、drawn 各行只
  掉 1-3 帧。转场窗口本来每次就有 20-29 次 overrun，此时 render_avg 对
  "每帧多画十几个小形状"异常敏感，却不等价于可感知的掉帧。
